...
; legacy 'unique' sold list. deprecated. kept around to allow moving the contents of the list to the new list in
...
Event OnUnload()
    ...
    _Log(fnName, "unregistering for player load game events", LL_INFO)
...
Function Initialize(ObjectReference akLandingMarkerRef)
    ...
    If akLandingMarkerRef != None
        ...
        _Log(fnName, "setting MyLandingMarker to " + MyLandingMarker, LL_INFO)
    Else
...
Function InitializeSVFEnhancementsVersion1()
    ...
    _Log(fnName, "Ship Vendor Framework enhancements initializing to version " + updatingToVersion, LL_INFO)
    ...
    If initialized == true
        _Log(fnName, "vendor already initialized, deleting existing ships and resetting timestamp to trigger inventory refresh", LL_INFO)
    ...
    _Log(fnName, "Ship Vendor Framework enhancements initialized to version " + updatingToVersion, LL_INFO)
...
Function InitializeSVFEnhancementsVersion2()
    ...
    _Log(fnName, "purging ShipsForSale* lists of ship references that are None", LL_INFO)
    LockGuard ShipsForSaleGuard
    ...
    _Log(fnName, "purging ship to leveled base ship maps of mappings that have ship references that are None", LL_INFO)
    ...
    If MyLandingMarker != None
        _Log(fnName, "purging linked ships that are owned by the vendor but don't have mappings and weren't sold by the player", LL_INFO)
    ...
    If initialized == true && lastInventoryRefreshTimestamp > 0.0
        _Log(fnName, "resetting timestamp to trigger inventory refresh", LL_INFO)
        ; reset timestamp so that the inventory refresh happens immediately
        lastInventoryRefreshTimestamp = 0.0
    EndIf
...
Function InitializeSVFEnhancementsVersion3()
    ...
    int controlInitTimeout = 3
    While svfControl.SVFControlInitialized() == false && controlInitTimeout > 0
        _Log(fnName, "Waiting for SVF Control to initialize...", LL_WARNING)
        Utility.WaitMenuPause(1.0)
        controlInitTimeout -= 1
    EndWhile

    ; spit out a log message if control script initialization timed out
    If controlInitTimeout == 0 && svfControl.SVFControlInitialized() == false
        _Log(fnName, "SVF Control initialization timed out", LL_ERROR)
    EndIf

    ; convert then clear the old uniques sold list
    int i = uniquesSoldListLocal.Length - 1
    While i >= 0
        svfControl.UniqueShipsSold.AddForm(uniquesSoldListLocal[i])
        i -= 1
    EndWhile
    uniquesSoldListLocal.Clear()
    uniquesSoldListLocal = None

    ; process vendor data map
    PopulateLocals()

    ; back up original values of BuysShips/SellsShips
    originalBuysShips = BuysShips
    originalSellsShips = SellsShips
...
Function UseSVFDatasetsCheck()
    ...
    ; prior to SVF installation, they may not be set to what we need.
    If useSVFDatasets == true && (BuysShips == false || SellsShips == false)
        _Log(fnName, "vendor had BuysShips set to " + BuysShips + " and SellsShips set to " + SellsShips + ", setting both to true", LL_WARNING)
        BuysShips = true
        SellsShips = true
    ElseIf useSVFDatasets == false
...
Event RefCollectionAlias.OnShipSold(RefCollectionAlias akSender, ObjectReference akSenderRef)
    ...
    If soldShip && soldShip.GetLinkedRef(SpaceshipStoredLink) == MyLandingMarker
        _Log(fnName, "player sold ship " + soldShip + " that has linked ref " + soldShip.GetLinkedRef(SpaceshipStoredLink) + "(landing marker is " + MyLandingMarker + ") for keyword " + SpaceshipStoredLink, LL_INFO)
        LockGuard ShipsForSaleGuard
...
Function CheckForInventoryRefresh(bool abForceRefresh = false)
    ...
    If SellsShips
        ...
        EndIf
    Else
        _Log(fnName, "vendor does not sell ships, skipping inventory refresh", LL_INFO)
    EndIf
...
Function PurgeAlreadySoldUniques(SpaceshipReference[] akShipList, SpaceshipReference[] akShipListUniques)
    ...
    ; only needs to run if there are unique ships to sell and there are unique ships that have already been sold
    LeveledSpaceshipBase[] uniqueShipsSoldLocal = svfControl.UniqueShipsSold.GetArray() as LeveledSpaceshipBase[]
    If UniqueShipsToSell() == true && uniqueShipsSoldLocal.Length > 0
...
Function RefreshInventoryList(ObjectReference akCreateMarker, SpaceshipReference[] akShipList, SpaceshipReference[] akShipListAlways, SpaceshipReference[] akShipListRandom, SpaceshipReference[] akShipListUnique, SpaceshipReference[] akShipListSoldByPlayer)
    ...
    If akCreateMarker
        ...
        _Log(fnName, "combining ship lists", LL_INFO)
        ...
        _Log(fnName, "combining ship ref to leveled ship mappings", LL_INFO)
        _Log(fnName, "shipsForSaleMapping=" + shipsForSaleMapping, LL_DEBUG)
        _Log(fnName, "shipsForSaleMappingAlways=" + shipsForSaleMappingAlways, LL_DEBUG)
        _Log(fnName, "shipsForSaleMappingRandom=" + shipsForSaleMappingRandom, LL_DEBUG)
        _Log(fnName, "shipsForSaleMappingUnique=" + shipsForSaleMappingUnique, LL_DEBUG)
...
Function CreateShipsForSale(var[] akShipToSellList, ObjectReference akCreateMarker, Location akEncLoc, SpaceshipReference[] akShipList, ShipRefToSpaceshipLeveledListMapping[] akRefToLLMap, int aiShipsToCreate = -1, bool abRandomize = false)
    ...
    If aiShipsToCreate == 0
        _Log(fnName, "no ships to create", LL_INFO)
        Return
    EndIf
    ...
    If abRandomize == true
        _Log(fnName, "randomizing ship list", LL_INFO)
...
Function CreateShipForSale(LeveledSpaceshipBase akShipToCreate, ObjectReference akCreateMarker, Location akEncLoc, SpaceshipReference[] akShipList, ShipRefToSpaceshipLeveledListMapping[] akRefToLLMap)
    ...
    If newShip != None && newShip.IsBoundGameObjectAvailable()
        ...
        _Log(fnName, "created ship " + newShip + " from leveled ship " + akShipToCreate, LL_INFO)
    Else
        _Log(fnName, "no ship created", LL_INFO)
    EndIf
